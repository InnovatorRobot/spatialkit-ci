...
Renderer::Renderer(int width, int height)
    : m_width(width), m_height(height), m_initialized(false), m_defaultFBO(0) {}
...
void Renderer::BeginFrame() { glViewport(0, 0, m_width, m_height); }
...
void Renderer::CaptureFramebuffer(std::vector<uint8_t> &pixels) {
  ...
  for (int y = 0; y < m_height / 2; ++y) {
    ...
    for (int x = 0; x < m_width * 4; ++x) {
      std::swap(pixels[top * m_width * 4 + x],
                pixels[bottom * m_width * 4 + x]);
    }
  }
}

bool Renderer::SaveFramebufferToFile(const std::string &path) {
  std::vector<uint8_t> pixels;
  CaptureFramebuffer(pixels);

  return stbi_write_png(path.c_str(), m_width, m_height, 4, pixels.data(),
                        m_width * 4) != 0;
}
