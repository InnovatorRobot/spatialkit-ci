...
class VisualRegressionTester:
    ...
    def compare_images(self, output_path: Path, golden_path: Path) -> Tuple[bool, float, Optional[np.ndarray]]:
        """
        ...
        if output_img.size != golden_img.size:
            print(f"Warning: Image size mismatch. Output: {output_img.size}, Golden: {golden_img.size}")
...
def main():
    ...
    parser.add_argument('--golden-dir', default='tests/visual/golden',
                       help='Directory containing golden reference images')
    parser.add_argument('--threshold', type=float, default=0.01,
                       help='Difference threshold (0.0-1.0)')
